...
<head>
  ...
  <style>
    ...
    /* จัดระเบียบช่องกรอกข้อมูลให้วางข้างกัน */
    .input-row {
      display: flex;
    ...
    #extractedOutput {
      ...
      color: #00c853; /* สีเขียวสว่างสำหรับผลลัพธ์ที่ตัดออกมา */
...
<body>
  ...
  <div class="input-row">
    <div class="input-field">
      <label>ข้อความแนวทาง:</label>
      <textarea id="textInput" rows="8" placeholder="พิมพ์ตัวเลขหรือข้อความ..." oninput="updatePreview()"></textarea>
    </div>
    <div class="input-field">
      <label>ผลการตัดตัวเลขวิน:</label>
      <textarea id="extractedOutput" rows="8" placeholder="จะแสดงผลเมื่อพิมพ์ 'วิน'..." readonly></textarea>
    </div>
  </div>
...
อาจารย์หมาน นำโชค</div>
  ...
  <script>
    ...
    function updatePreview() {
      ...
      // --- ส่วนดึงข้อมูลวินและจัดรูปแบบ พร้อมเพิ่มชื่อประเภทหวยด้านหน้า ---
      const lines = text.split('\n');
      let resultText = "";
      
      lines.forEach(line => {
        if (line.includes("วิน")) {
          // ดึงเฉพาะตัวเลขออกมาจากบรรทัดนั้น
          const nums = line.replace(/[^0-9]/g, ''); 
          if (nums.length >= 4) {
            // ตัด 4 ตัวแรกตามเงื่อนไข (1, 2 = รูด / 3, 4 = จับคู่)
            const r1 = nums[0];
            const r2 = nums[1];
            const pair = nums.substring(2, 4);
            // เพิ่มตัวแปร type ไว้หน้า รูด
            resultText = `${type} รูด ${r1}-${r2} / จับคู่รูดกับ ${pair}`;
          }
        }
      });
      document.getElementById('extractedOutput').value = resultText;
      // -------------------------------
